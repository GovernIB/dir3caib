...
public class ImportadorOficinasBean extends ImportadorBase implements ImportadorOficinasLocal {
    ...
    public void importarSincronizacion(Sincronizacion sincronizacion) throws Exception {

        System.gc();

        // Inicializamos la cache para la importación de Oficinas
        cacheImportadorOficinas(true, sincronizacion);

        try {
            String directorio = Configuracio.getDirectorioPath(sincronizacion.getCodigo());

            log.info("Inicio sincronización incremental OFICINAS");
            log.info("------------------------------------------------");
            log.info("Directorio: " + directorio);

            // Oficinas
            importarCSVOficinas(directorio);
            ...
            importarCSVRelacionesOrganizativas(directorio);

            // Relaciones SIR Oficina
            importarCSVRelacionesSir(directorio);

            //Servicios
            importarCSVServicios(directorio);

            System.gc();

            log.info("");
            log.info("Fin sincronización incremental OFICINAS");
    ...
    public void importarCompleta(Sincronizacion sincronizacion) throws Exception {
        ...
        // Inicializamos la cache para la importación de Oficinas
        cacheImportadorOficinas(false, sincronizacion);

        log.info("Inicio sincronizacion completa OFICINAS");
        log.info("------------------------------------------------");

        try {

            // Importamos las Oficinas de todos los Niveles
            for (CatNivelAdministracion nivel : niveles) {

                log.info("------------------------------------------------");
                log.info("Inicio importacion de Oficinas de nivel administracion: " + nivel.getCodigoNivelAdministracion() + " - " + nivel.getDescripcionNivelAdministracion());
            ...
            log.info("");
            log.info("Fin sincronizacion completa OFICINAS");
    ...
    private void importarCSVOficinas(String directorio) throws Exception {

        // Obtenemos el fichero del sistema de archivos
        String fichero = Dir3caibConstantes.OFI_OFICINAS;
    ...
    private void importarCSVHistoricos(String directorio) throws Exception {
        ...
        // Obtenemos el fichero del sistema de archivos
        String fichero = Dir3caibConstantes.OFI_HISTORICOS_OFI;
        CSVReader reader = getReader(directorio, fichero);
    ...
    private void importarCSVContactos(String directorio) throws Exception {
        ...
        // Obtenemos el fichero del sistema de archivos
        String fichero = Dir3caibConstantes.OFI_CONTACTO_OFI;
        CSVReader reader = getReader(directorio, fichero);
     ...
     * @throws Exception
     */
    private void importarCSVRelacionesOrganizativas(String directorio) throws Exception {

        // Obtenemos el fichero del sistema de archivos
        String fichero = Dir3caibConstantes.OFI_RELACIONES_ORGANIZATIVAS_OFI;
        CSVReader reader = getReader(directorio, fichero);
     ...
     * @throws Exception
     */
    private void importarCSVRelacionesSir(String directorio) throws Exception {

        // Obtenemos el fichero del sistema de archivos
        String fichero = Dir3caibConstantes.OFI_RELACIONES_SIROFI;
        CSVReader reader = getReader(directorio, fichero);
    ...
    private void importarCSVServicios(String directorio) throws Exception {

        // Obtenemos el fichero del sistema de archivos
        String fichero = Dir3caibConstantes.OFI_SERVICIOS_OFI;
        CSVReader reader = getReader(directorio, fichero);
    ...
    private void componerOficina(Oficina oficina, String[] fila) throws Exception {
        ...
        String codUoResponsableVersion = codUOResponsable + "v" + versionUOResponsable;
        if (!codUoResponsableVersion.isEmpty()) {
            oficina.setCodUoResponsable(cacheUnidad.get(codUoResponsableVersion));
        } else {
