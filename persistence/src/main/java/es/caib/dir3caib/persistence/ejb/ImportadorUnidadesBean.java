...
public class ImportadorUnidadesBean extends ImportadorBase implements ImportadorUnidadesLocal {
    ...
    public void importarUnidades(Sincronizacion sincronizacion) throws Exception {
        ...
        if (sincronizacion.getTipo().equals(Dir3caibConstantes.DIRECTORIO_ACTUALIZACION)) {

            importarSincronizacion(sincronizacion);

        } else if (sincronizacion.getTipo().equals(Dir3caibConstantes.DIRECTORIO_COMPLETO)) {

            importarCompleta(sincronizacion);

        }
    }

    /**
     * Realiza una importanción de una Sincronización incremental del directorio
     *
     * @param sincronizacion
     * @throws Exception
     */
    private void importarSincronizacion(Sincronizacion sincronizacion) throws Exception {
        ...
        try {
            ...
            String directorio = Configuracio.getDirectorioPath(sincronizacion.getCodigo());
            log.info("Inicio Sincronización incremental Unidades");
            log.info("------------------------------------------------");
            log.info("Directorio: " + directorio);

            // Unidades
            importarCSVUnidades(directorio);

            // Historicos
            importarCSVHistoricos(directorio);

            // Contactos
            importarCSVContactos(directorio);

            // CodigosUO
            importarCSVCodigosUO(directorio);

            // NifCifUO
            importarCSVNifCifUO(directorio);

            // ServiciosUO
            importarCSVServiciosUO(directorio);
        ...
        log.info("");
        log.info("Fin Sincronización incremental Unidades");

    }

    /**
     * Realiza una importanción de una Sincronización Completa del directorio
     *
     * @param sincronizacion
     * @throws Exception
     */
    private void importarCompleta(Sincronizacion sincronizacion) throws Exception {
        ...
        log.info("Inicio importacion completa");
        log.info("------------------------------------------------");
        try {
            // Importamos las Unidades de todos los Niveles
            for (CatNivelAdministracion nivel : niveles) {

                log.info("------------------------------------------------");
                log.info("Inicio importacion de Unidades de nivel administracion: " + nivel.getCodigoNivelAdministracion() + " - " + nivel.getDescripcionNivelAdministracion());
            ...
            // Procesamos el resto de Ficheros de las Unidades
            for (CatNivelAdministracion nivel : niveles) {
                log.info("Inicio importacion completa resto de ficheros de nivel administracion: " + nivel.getCodigoNivelAdministracion() + " - " + nivel.getDescripcionNivelAdministracion());
                String directorio = Configuracio.getDirectorioPath(sincronizacion.getCodigo()) + "nivel_" + nivel.getCodigoNivelAdministracion() + "/";
            ...
            log.info("");
            log.info("Fin importacion completa UNIDADES");
     ...
     * @throws Exception
     */
    private void importarCSVUnidades(String directorio) throws Exception {

        // Obtenemos el fichero del sistema de archivos
        String fichero = Dir3caibConstantes.UO_UNIDADES;
        CSVReader reader = getReader(directorio, fichero);
     ...
     * @throws Exception
     */
    private void importarCSVHistoricos(String directorio) throws Exception {

        // Obtenemos el fichero del sistema de archivos
        String fichero = Dir3caibConstantes.UO_HISTORICOS_UO;
        CSVReader reader = getReader(directorio, fichero);

        if (reader != null) {

            log.info("");
            log.info("Inicio importacion: " + fichero);
            log.info("------------------------------------");
     ...
     * @param directorio
     */
    private void importarCSVContactos(String directorio) throws Exception {

        // Obtenemos el fichero del sistema de archivos
        String fichero = Dir3caibConstantes.UO_CONTACTO_UO;
        CSVReader reader = getReader(directorio, fichero);
     ...
     *
     * @param directorio que contiene los códigos de fuente externa
     */
    private void importarCSVCodigosUO(String directorio) throws Exception {

        // Obtenemos el fichero del sistema de archivos
        String fichero = Dir3caibConstantes.UO_CODIGO_UO;
        CSVReader reader = getReader(directorio, fichero);
     ...
     *
     * @param directorio que contiene los nifcif de fuente externa
     */
    private void importarCSVNifCifUO(String directorio) throws Exception {

        // Obtenemos el fichero del sistema de archivos
        String fichero = Dir3caibConstantes.UO_NIFCIF_UO;
        CSVReader reader = getReader(directorio, fichero);
     ...
     * @throws Exception
     */
    private void importarCSVServiciosUO(String directorio) throws Exception {

        // Obtenemos el fichero del sistema de archivos
        String fichero = Dir3caibConstantes.UO_SERVICIOS_UO;
        CSVReader reader = getReader(directorio, fichero);
