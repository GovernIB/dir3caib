...
public class SincronizacionBean extends BaseEjbJPA<Sincronizacion, Long> implements SincronizacionLocal {
    ...
    public Sincronizacion ultimaSincronizacionDirectorio() throws Exception {
        ...
        Query query = em.createQuery("select sincronizacion from Sincronizacion as sincronizacion where (sincronizacion.tipo = :completa or sincronizacion.tipo = :actualizacion) " +
    ...
    /* El funcionamiento de los ws de madrid no permiten que la fecha de inicio sea null si la fecha fin es distinta de null.
       ...
       fecha de inicio y fecha fin. Esta devuelve la información añadida o modificada entre esas dos fechas.*/
        ...
        try {
            ...
            String unidadesZip = sincronizacionPath + Dir3caibConstantes.UNIDADES_ARCHIVO_ZIP + sincronizacion.getCodigo() + ".zip";
            String oficinasZip = sincronizacionPath + Dir3caibConstantes.OFICINAS_ARCHIVO_ZIP + sincronizacion.getCodigo() + ".zip";
            ...
            } else if (codigoUnidades.equals(Dir3caibConstantes.CODIGO_CORRECTO) || codigoOficinas.equals(Dir3caibConstantes.CODIGO_CORRECTO)) {
    ...
    /* El funcionamiento de los ws de madrid no permiten que la fecha de inicio sea null si la fecha fin es distinta de null.
       ...
       fecha de inicio y fecha fin. Esta devuelve la información añadida o modificada entre esas dos fechas.*/
        ...
        try {
            ...
            // Realizamos la descarga por niveles para reducir la carga de la peticióna  los WS
            for (CatNivelAdministracion nivel : niveles) {
                ...
                //parametrosOficinas.setEstados(estados); TODO hay un error en la definición del API

                // Invocamos el WS de Unidades
                log.info("Peticion WS unidades DIR3 Nivel administracion: " + nivel.getCodigoNivelAdministracion().intValue() + " - " + nivel.getDescripcionNivelAdministracion());
                es.caib.dir3caib.ws.dir3.unidad.client.RespuestaWS respuestaUnidades = serviceUnidades.exportarV3(parametrosUnidades);
                log.info("Respuesta WS unidades DIR3 para nivel " + nivel.getDescripcionNivelAdministracion() + ": " + respuestaUnidades.getCodigo() + " - " + respuestaUnidades.getDescripcion());

                String codigoUnidades = respuestaUnidades.getCodigo().trim();

                // Invocamos el WS de Oficinas
                log.info("Peticion WS oficinas DIR3 Nivel administracion: " + nivel.getCodigoNivelAdministracion().intValue() + " - " + nivel.getDescripcionNivelAdministracion());
                es.caib.dir3caib.ws.dir3.oficina.client.RespuestaWS respuestaOficinas = serviceOficinas.exportarV4(parametrosOficinas);
                log.info("Respuesta WS oficinas DIR3 para nivel " + nivel.getDescripcionNivelAdministracion() + ": " + respuestaOficinas.getCodigo() + " - " + respuestaOficinas.getDescripcion());
