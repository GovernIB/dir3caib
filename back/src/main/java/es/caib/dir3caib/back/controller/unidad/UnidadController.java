    ...
    private ModelAndView detalleComun(String codUnidad, boolean denominacionCooficial) throws Exception {
        ...
        if (Dir3caibConstantes.ESTADO_ENTIDAD_VIGENTE.equals(unidad.getEstado().getCodigoEstadoEntidad())) {
            ...
            if (unidad.getNivelJerarquico() != 0) {
                ...
                unidadesSegundoNivel.addAll(unidadEjb.getUnidadesByNivelByUnidadSuperior(nivelJerarquicoInicial + 1, unidad.getCodigo(), Dir3caibConstantes.ESTADO_ENTIDAD_VIGENTE, denominacionCooficial));
                if (unidadesSegundoNivel.size() != 0) {
                    for (Unidad unidad2 : unidadesSegundoNivel) {
                        unidadesTercerNivel.addAll(unidadEjb.getUnidadesByNivelByUnidadSuperior(nivelJerarquicoInicial + 2, unidad2.getCodigo(), Dir3caibConstantes.ESTADO_ENTIDAD_VIGENTE, denominacionCooficial));
                    }
                    if (unidadesTercerNivel.size() != 0) {
                        for (Unidad unidad3 : unidadesTercerNivel) {
                            unidadesCuartoNivel.addAll(unidadEjb.getUnidadesByNivelByUnidadSuperior(nivelJerarquicoInicial + 3, unidad3.getCodigo(), Dir3caibConstantes.ESTADO_ENTIDAD_VIGENTE, denominacionCooficial));
                        }
                        if (unidadesCuartoNivel.size() != 0) {
                            for (Unidad unidad4 : unidadesCuartoNivel) {
                                unidadesQuintoNivel.addAll(unidadEjb.getUnidadesByNivelByUnidadSuperior(nivelJerarquicoInicial + 4, unidad4.getCodigo(), Dir3caibConstantes.ESTADO_ENTIDAD_VIGENTE, denominacionCooficial));
                            }
                            if (unidadesQuintoNivel.size() != 0) {
                                for (Unidad unidad5 : unidadesQuintoNivel) {
                                    unidadesSextoNivel.addAll(unidadEjb.getUnidadesByNivelByUnidadSuperior(nivelJerarquicoInicial + 5, unidad5.getCodigo(), Dir3caibConstantes.ESTADO_ENTIDAD_VIGENTE, denominacionCooficial));
                                }
                                if (unidadesSextoNivel.size() != 0) {
                                    for (Unidad unidad6 : unidadesSextoNivel) {
                                        unidadesSeptimoNivel.addAll(unidadEjb.getUnidadesByNivelByUnidadSuperior(nivelJerarquicoInicial + 6, unidad6.getCodigo(), Dir3caibConstantes.ESTADO_ENTIDAD_VIGENTE, denominacionCooficial));
                                    }
                                }
                            }
                        }
                ...
                unidadesPrimerNivel = unidadEjb.getUnidadesByNivelByUnidadSuperior((long) 1, unidad.getCodigo(), Dir3caibConstantes.ESTADO_ENTIDAD_VIGENTE, denominacionCooficial);
                for (Unidad unidad1 : unidadesPrimerNivel) {
                    unidadesSegundoNivel.addAll(unidadEjb.getUnidadesByNivelByUnidadSuperior((long) 2, unidad1.getCodigo(), Dir3caibConstantes.ESTADO_ENTIDAD_VIGENTE, denominacionCooficial));
                }
                for (Unidad unidad2 : unidadesSegundoNivel) {
                    unidadesTercerNivel.addAll(unidadEjb.getUnidadesByNivelByUnidadSuperior((long) 3, unidad2.getCodigo(), Dir3caibConstantes.ESTADO_ENTIDAD_VIGENTE, denominacionCooficial));
                }
            }
            ...
            if (unidad.getNivelJerarquico() != 0) {
                oficinasPrincipales = oficinaEjb.responsableByUnidadEstado(unidad.getCodUnidadRaiz().getCodigo(), Dir3caibConstantes.ESTADO_ENTIDAD_VIGENTE, denominacionCooficial);
                oficinasAuxiliares = oficinaEjb.dependienteByUnidadEstado(unidad.getCodUnidadRaiz().getCodigo(), Dir3caibConstantes.ESTADO_ENTIDAD_VIGENTE, denominacionCooficial);
                relacionesOrganizativaOfi = relacionOrganizativaOfiEjb.getOrganizativasCompletoByUnidadEstado(unidad.getCodUnidadRaiz().getCodigo(), Dir3caibConstantes.ESTADO_ENTIDAD_VIGENTE, denominacionCooficial);
                relacionesSirOfi = relacionSirOfiEjb.relacionesSirOfiByUnidaddEstado(unidad.getCodUnidadRaiz().getCodigo(), Dir3caibConstantes.ESTADO_ENTIDAD_VIGENTE, denominacionCooficial);
            }
